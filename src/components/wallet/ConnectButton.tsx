...
export default function ConnectButton() {
  const { account, balance, isConnecting, isCorrectNetwork, connect, disconnect } = useWeb3();
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className={`w-2 h-2 rounded-full ${isCorrectNetwork ? 'bg-green-400' : 'bg-amber-400'} animate-pulse`} />
      ...
      {showDropdown && (
        <div className="absolute right-0 mt-2 w-64 bg-dark-800 border border-dark-700 rounded-lg shadow-xl z-50 animate-fade-in">
          <div className="p-4 border-b border-dark-700">
            ...
            <div className="text-sm font-mono text-dark-200 break-all">{account}</div>
          </div>
