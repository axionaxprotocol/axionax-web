...
export default function InfrastructurePage() {
  const services = [
    { name: 'RPC Node (HTTP)', port: 8545, status: 'healthy', uptime: '18h+', details: 'Mock JSON-RPC 2.0' },
    { name: 'RPC Node (WebSocket)', port: 8546, status: 'healthy', uptime: '18h+', details: 'Real-time updates' },
    { name: 'PostgreSQL', port: 5432, status: 'healthy', uptime: '44h+', details: 'Database layer' },
    { name: 'Redis', port: 6379, status: 'healthy', uptime: '44h+', details: 'Cache layer' },
    { name: 'Nginx', port: 80, status: 'healthy', uptime: '44h+', details: 'Web server' },
    { name: 'Nginx SSL', port: 443, status: 'healthy', uptime: '44h+', details: 'HTTPS encryption' },
    { name: 'Grafana', port: 3030, status: 'healthy', uptime: '15h+', details: 'Monitoring v12.2.1' },
    { name: 'Prometheus', port: 9090, status: 'healthy', uptime: '15h+', details: 'Metrics collection' },
    { name: 'Web Interface', port: 3000, status: 'healthy', uptime: '23h+', details: 'axionax-web' },
    { name: 'Explorer API', port: 3001, status: 'debugging', uptime: 'N/A', details: 'Under investigation' },
    { name: 'Faucet API', port: 3002, status: 'debugging', uptime: 'N/A', details: 'Under investigation' },
  ];
      ...
      <main className="section pt-32">
        <div className="container-custom">
          ...
          <div className="text-center mb-12">
            ...
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-dark-900 border border-dark-800 rounded-xl">
              <div className="flex items-center gap-2">
                ...
                <span className="text-amber-400 font-semibold">7/9 Services Operational</span>
              </div>
          ...
          <div className="mb-12 p-6 rounded-xl bg-gradient-to-r from-primary-900/20 to-secondary-900/20 border border-primary-500/20">
            <h2 className="text-2xl font-bold text-dark-100 mb-4">VPS Information</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className="p-6 rounded-xl bg-dark-900 border border-dark-800">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-dark-200">RAM Usage</h3>
            ...
            <div className="p-6 rounded-xl bg-dark-900 border border-dark-800">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-dark-200">CPU Load</h3>
                <span className="text-2xl">⚡</span>
              </div>
              ...
              <div className="text-sm text-dark-400 mt-2">&lt; 1.0 load average</div>
            </div>

            <div className="p-6 rounded-xl bg-dark-900 border border-dark-800">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-dark-200">Disk Space</h3>
                <span className="text-2xl">💿</span>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border border-dark-800 rounded-xl overflow-hidden">
              <thead className="bg-dark-900">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-dark-300">Service</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-dark-300">Port</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-dark-300">Status</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-dark-300">Uptime</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-dark-300">Details</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-dark-800">
                {services.map((service) => (
                  <tr key={`${service.name}-${service.port}`} className="hover:bg-dark-900/50 transition-colors">
                    <td className="px-6 py-4 text-dark-200 font-medium">{service.name}</td>
                    <td className="px-6 py-4 text-dark-300 font-mono">{service.port}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(service.status)}`}>
                        ...
                        <span>{service.status === 'healthy' ? 'Healthy' : 'Debugging'}</span>
                      </span>
                    </td>
                    <td className="px-6 py-4 text-dark-300">{service.uptime}</td>
                    <td className="px-6 py-4 text-dark-400 text-sm">{service.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
            <div className="p-6 rounded-xl bg-dark-900 border border-green-500/20">
              <h3 className="text-lg font-semibold text-green-400 mb-4">✅ Infrastructure Layer</h3>
            ...
            <div className="p-6 rounded-xl bg-dark-900 border border-green-500/20">
              <h3 className="text-lg font-semibold text-green-400 mb-4">✅ Monitoring Stack</h3>
            ...
            <div className="p-6 rounded-xl bg-dark-900 border border-amber-500/20">
              <h3 className="text-lg font-semibold text-amber-400 mb-4">🔧 Application Layer</h3>
              <div className="text-3xl font-bold text-amber-400 mb-2">0/2</div>
              <p className="text-dark-400 text-sm mb-4">Debugging in Progress</p>
              <ul className="space-y-2 text-sm text-dark-300">
          ...
          <div className="mt-12 p-6 rounded-xl bg-dark-900 border border-dark-800">
            <h3 className="text-lg font-semibold text-dark-200 mb-4">📚 Documentation</h3>
